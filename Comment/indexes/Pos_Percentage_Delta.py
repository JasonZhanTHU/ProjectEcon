...
with open('../files/POS_ad_reviews_date.json', 'r') as f:
    ad = json.load(f)

with open('../files/POS_ad_EU_reviews_date.json', 'r') as f:
    ad_EU = json.load(f)
...
for i in range(-event_window_before, event_window_after + 1):
    ...
    window_end = window_start + pd.DateOffset(days=1)
...
# plt.plot(x_axis, excess_returns1, marker='o', linestyle='-')
# plt.plot(x_axis, excess_returns2, marker='o', linestyle='-')
